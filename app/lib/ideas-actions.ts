...
const IdeaSchema = z.object({
    ...
    status: z.enum(['High Potential', 'Active Test', 'Paused', 'High Alpha', 'Draft']).default('Draft'),
...
export async function createIdea(prevState: State, formData: FormData) {
    const validatedFields = CreateIdea.safeParse({
        ...
        categories: formData.get('categories') ? (formData.get('categories') as string).split(',').map(s => s.trim()) : [],
        keywords: formData.get('keywords') ? (formData.get('keywords') as string).split(',').map(s => s.trim()) : [],
