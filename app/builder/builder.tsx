...
export default function Builder() {
  ...
  const handleAutowireZodSchema = (tableNode: any) => {
    ...
    const mappedFields = columns.map((col: any) => {
      ...
      let zodStr = 'z.string()'; // Default validation type

      // Map data types to Zod base configurations
      if (
        ...
        zodStr = isRequired
          ? `z.coerce.number({ invalid_type_error: 'Please enter a valid number for ${col.name}.' })`
          : 'z.coerce.number()';
      } else if (sqlType.includes('BOOL')) {
        zodStr = 'z.boolean()';
      } else if (col.name.toLowerCase().includes('email')) {
        // Special case: Automatically configure emails with native validation
        zodStr = isRequired
      ...
      // Add nullability/optional logic to the base string
      if (isAuto) {
        zodStr += '.optional()'; // Auto-generated fields aren't required in base inputs
      } else if (!isRequired) {
        zodStr += '.nullable().optional()'; // Nullable fields can be omitted or null
      } else if (zodStr === 'z.string()') {
        // Add custom required error message to plain strings
    ...
    // 3. Create the new Zod Schema node with smart configurations
    const newZodSchemaNode = {
      id,
      type: 'zodSchema',
      position: {
        x: tableNode.position.x + 350, // Neatly offset to the right of DB Table
        ...
        operations: [
          {
            ...
            omitFields: autoGeneratedFields, // Pre-populate omit with auto-fields (like serial ID, timestamps)
          },
          {
            name: `Update${pascalName}`,
            omitFields: primaryKeys, // Pre-populate update omit list with primary keys
          },
        ],
      },
    };
    ...
    setSelectedNodeId(id); // Set focus to the new schema sidebar panel
  };
