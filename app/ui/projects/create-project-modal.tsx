...
export default function CreateProjectModal({
  ...
  const calculateWorkingDays = (tasks: Partial<Task>[], sum: number): number => {
    ...
    const subTaskTotals = tasks.reduce(
      (total, task) => total + calculateWorkingDays(task?.sub_tasks || [], 0),
      0
    );
    ...
    <form action={handleSubmit} className="flex flex-1 overflow-y-auto flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8 group">
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold uppercase tracking-widest text-slate-400 px-1 font-manrope">
            Project Title
          </label>
          <div className="relative">
            <input
              ...
              value={projectName}
              className="w-full bg-slate-100/60 border-none rounded-xl px-4 py-4 font-manrope font-semibold text-slate-800 placeholder:text-slate-400 focus:ring-2 focus:ring-[#0EA5E9]/20 transition-all shadow-xs shadow-slate-200/50"
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <textarea
              ...
              className="w-full bg-slate-100/60 border-none rounded-xl px-4 py-4 font-manrope text-sm text-slate-600 placeholder:text-slate-400 focus:ring-2 focus:ring-[#0EA5E9]/20 transition-all resize-none shadow-xs shadow-slate-200/50"
              placeholder="Describe the strategic objectives..."
              rows={4}
            ></textarea>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={autoGenerateHierarchy}
              className="text-sky-600 text-sm font-semibold hover:text-sky-700 flex items-center gap-1"
            >
          ...
          <div className="space-y-6 relative">
            ...
            <button
              type="button"
              onClick={addNewTask}
              className="flex items-center gap-3 px-4 py-3 w-full border border-dashed border-slate-200 rounded-xl text-slate-400 hover:text-sky-600 hover:border-sky-300 hover:bg-sky-50 transition-all font-medium text-sm group mt-6"
            >
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="p-4 rounded-xl bg-sky-50/50 border border-sky-100/50">
            <p className="text-[10px] font-black uppercase tracking-widest text-sky-700/50">
              Estimated Duration
            </p>
            <p className="text-xl font-bold text-sky-900">
              {calculateWorkingDays(tasks, 0)} Working Days
            </p>
          </div>
      ...
      <div className="px-8 py-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between rounded-b-2xl">
        ...
        <div className="flex items-center gap-4">
          <button
            type="button"
            className="px-6 py-2.5 font-bold text-sky-700 hover:bg-sky-100 rounded-lg transition-colors"
          >
            Save as Template
          </button>
